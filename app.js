...
let NetworkMonitor = function (config) {
    ...
    const generateHash = function (num) {
        let table = ['1', '2', '3', '4', '5', '6', '7', '8', '9', 'a', 'b', 'c', 'd', 'e', 'f']
        let hash = ''
        for (let i = 0; i < num; i++) {
    ...
    let addNodeInterval = setInterval(() => {
        getNode()
        nodeCount += 1
        if (nodeCount > 20) clearInterval(addNodeInterval)
    }, 6000)
    ...
    const injectTransactions = function () {
        for (let nodeId in G.active) {
            ...
            let injectInterval = setInterval(() => {
                ...
                let travelDistance = distanceBtnTwoNodes(injectedTx, node, true)
                transformCircle(injectedTx.circle, travelDistance.x, travelDistance.y, null, G.txAnimationSpeed)
                setTimeout(() => {
                    injectedTx.currentPosition = injectedTx.circle.getCenterPoint()
    ...
    const updateUI = function (previousStatus, currentStatus, publicKey, nodeId) {
        if (previousStatus === 'joining' && currentStatus === 'syncing') {
            ...
            node.circle.set('fill', G.colors['active'])
        }
    }
    ...
    const relocateIntoNetwork = function (previousStatus, node) {
        if (previousStatus === 'joining') {
            ...
            transformCircle(circle, x, y, G.colors['syncing'], 500)
    ...
    const positionNewNodeIntoNetwork = function (currentStatus, node) {
        if (currentStatus === 'syncing' || currentStatus === 'active') {
            ...
            let x = networkPosition.x
            let y = networkPosition.y
            let initialX = node.circle.getCenterPoint().x
            let initialY = node.circle.getCenterPoint().y
            let travelX
            let travelY
    ...
    const createNewNode = function (type, id) {
        ...
        if (type === 'joining') {
            ...
            let node = {
                circle: circle,
                status: type,
                currentPosition: circle.getCenterPoint()
            }
    ...
    const createNewTxCircle = function (inputTx, toNode) {
        ...
        let circle = drawCircle({ x: x, y: y }, 5, G.colors['transaction'], 2)
        let currentPosition = {
    ...
    const drawStateCircle = function (node) {
        ...
        }, radius, `#${node.appState.slice(0, 6)}`)
        return stateCircle
    }
    ...
    const drawCycleMarkerBox = function (node) {
        ...
        let cycleMarkerCircle = drawCircle({
            ...
            y: node.currentPosition.y - radius
        }, radius, `#${node.cycleMarker.slice(0, 6)}`)
    ...
    const drawNodeListBox = function (node) {
        ...
        let nodeListCircle = drawCircle({
            x: node.currentPosition.x - radius,
            y: node.currentPosition.y - radius
        }, radius, `#${node.nodelistHash.slice(0, 6)}`)
    ...
    function transformCircle(circle, x, y, fill, duration) {
        ...
        circle.animate({ 'left': x - radius, 'top': y - radius }, {
            duration: duration,
    ...
    const distanceBtnTwoNodes = function (node1, node2, substract) {
        ...
        // }
        return {
            x: node2.currentPosition.x,
            y: node2.currentPosition.y
        }
    }
    ...
    const getRandomActiveNodes = function (count, excludedNode = null) {
        ...
        let randomNodes = []
        let n
        if (excludedNode) nodeList = nodeList.filter(n => JSON.stringify(n) !== JSON.stringify(excludedNode))
        ...
        for (let i = 0; i < n; i += 1) {
            ...
            nodeList = nodeList.filter(n => JSON.stringify(n) !== JSON.stringify(excludedNode))
        }
        return randomNodes
    }
    ...
    let addImageToCanvas = async function (url, left, right, angle) {
    ...
    let removeImageFromCanvas = function (image) {
        canvas.remove(image)
    }
    ...
    const drawNetworkCycle = async function (R, X, Y) {
        ...
        const canvasWidth = G.VW
        const canvasHeight = G.VH

        let networkCenter = { x: canvasWidth / 2, y: canvasHeight / 2 }
        canvas.setHeight(canvasHeight)
        canvas.setWidth(canvasWidth)
        drawCircle(networkCenter, G.R, '#ffffff')
        $('#app').innerHTML = networkHTML

        let earth = await addImageToCanvas('earth.png', X - R, Y - R, 0)
    }
