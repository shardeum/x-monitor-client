...
let NetworkMonitor = function (config) {
                    ...
                    if (G.syncing[nodeId]) {
                        ...
                        try {
                            ...
                            G.active[nodeId].nodelistHash =
                                report.active[nodeId].nodelistHash;
            ...
            let injectInterval = setInterval(() => {
                ...
                setTimeout(() => {
                    ...
                    for (let i = 0; i < randomNodes.length; i += 1) {
                        ...
                        forwardInjectedTx(clonedTx, randomNodes[i]);
                    }
    ...
    const drawTooltip = function (node) {
        ...
        node.circle.on('mouseover', () => {
            ...
            node.textList.push(drawText(`nodeId: ${nodeIdShort}`, {
                x: position.x + marginLeft,
                y: position.y + marginBottom
            }, 13, "#ffffff"))
            node.textList.push(drawText(`marker: ${cycleMarkerShort}`, {
                x: position.x + marginLeft,
                y: position.y + marginBottom * 2
            }, 13, "#ffffff"))
            node.textList.push(drawText(`state: ${appStateShort}`, {
                x: position.x + marginLeft,
                y: position.y + marginBottom * 3
            }, 13, "#ffffff"))
            node.textList.push(drawText(`nodeList: ${nodeListShort}`, {
                x: position.x + marginLeft,
                y: position.y + marginBottom * 4
            }, 13, "#ffffff"))
            node.textList.push(drawText(`ExtIp: ${node.externalIp}`, {
                x: position.x + marginLeft,
                y: position.y + marginBottom * 5
            }, 13, "#ffffff"))
            node.textList.push(drawText(`ExtPort: ${node.externalPort}`, {
                x: position.x + marginLeft,
    ...
    const createNewNode = function (type, id) {
        ...
        if (type === "joining") {
            ...
            circle = drawCircle({
                x: 0,
                y: 0
            }, G.nodeRadius, G.colors["joining"], 2, id, 0.1);
            let node = {
                circle: circle,
                status: type,
    ...
    const forwardInjectedTx = function (injectedTx, targetNode) {
        let endPoint = distanceBtnTwoNodes(injectedTx, targetNode, true);
        let dur = Math.sqrt(endPoint.x ** 2 + endPoint.y ** 2);
        if (dur < 100) dur = 100;
        else dur = dur;
        transformCircle(injectedTx.circle, endPoint.x, endPoint.y, null, dur);
        setTimeout(() => {
            injectedTx.circle.graphics.clear();
            stage.update();
            injectedTx = null;
